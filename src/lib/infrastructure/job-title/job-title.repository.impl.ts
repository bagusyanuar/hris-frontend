import type { IJobTitleRepository, JobTitleModel, CreateJobTitleInput, UpdateJobTitleInput, JobTitleParams } from '$lib/core/job-title';
...
export class JobTitleRepositoryImpl implements IJobTitleRepository {
	...
	async getById(id: string): Promise<JobTitleModel | null> {
		return handleAppError(async () => {
			const response = await httpClient.get<ApiResponse<JobTitleResponse>>(`${this.basePath}/${id}`);
	...
	async update(input: UpdateJobTitleInput): Promise<JobTitleModel> {
		return handleAppError(async () => {
			...
			const response = await httpClient.put<ApiResponse<JobTitleResponse>>(`${this.basePath}/${input.id}`, request);
			return JobTitleMapper.toModel(response.data);
		});
	}
