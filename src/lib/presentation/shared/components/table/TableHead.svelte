<script lang="ts" generics="TData, TValue">
  ...
  let { header, align, pinned, class: className = '', children, ...restProps }: Props = $props();

  const context = getContext<{ density: 'default' | 'compact'; table?: Table<TData> }>(
    'TABLE_CONTEXT'
  );
  ...
  const HeaderComponent = $derived(
    isHeaderFunction ? flexRender(header!.column.columnDef.header, header!.getContext()) : null
  );
