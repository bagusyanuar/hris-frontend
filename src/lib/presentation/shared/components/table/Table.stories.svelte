  ...
  const freezeColumns: ColumnDef<Employee>[] = [
    {
      ...
      header: 'Nama',
      size: 200,
      meta: { pinned: 'left', className: 'font-medium' }
    },
...
<Story name="Custom Loading Element">
  <div class="flex flex-col gap-4 p-4">
    ...
    <TableUI.Root table={customLoadingTable.current} isLoading={true}>
      ...
      {#snippet loading()}
        <div
          class="flex flex-col items-center justify-center gap-3 rounded-xl border border-neutral-border bg-neutral-card p-8 shadow-sm"
        >
          <div class="flex gap-1.5">
            {#each [0, 1, 2] as dot (dot)}
              <span
                class="h-2.5 w-2.5 animate-bounce rounded-full bg-brand-primary"
                style="animation-delay: {dot * 0.15}s"
              ></span>
            {/each}
          </div>
          <Typography variant="body-sm" color="secondary"
            >Sedang mengambil data karyawan…</Typography
          >
        </div>
      {/snippet}
    </TableUI.Root>
...
<Story name="Custom No Data Element">
  <div class="flex flex-col gap-4 p-4">
    ...
    <TableUI.Root table={customEmptyTable.current}>
      ...
      {#snippet empty()}
        <div
          class="flex flex-col items-center justify-center border-t border-neutral-border p-12 text-center"
        >
...
<Story name="Row Index">
  <div class="flex flex-col gap-4 p-4">
    ...
    <Typography variant="body-sm" color="secondary">
      Kolom nomor urut otomatis via createRowNumberColumn(). Nomor ikut pagination & tetap benar
      setelah sorting.
    </Typography>
              ...
              <TableUI.Cell colspan={row.getVisibleCells().length}>
                <div class="flex flex-col gap-1 rounded-lg bg-neutral-bg/40 p-4">
                  <Typography variant="body-sm" weight="semibold"
                    >Detail: {row.original.name}</Typography
                  >
...
<Story name="Freeze Column">
  <div class="flex flex-col gap-4 p-4">
    ...
    <Typography variant="body-sm" color="secondary">
      Kolom Nama & Email di-freeze ke kiri (cumulative offset), Status di-freeze ke kanan. Scroll
      horizontal untuk melihat efeknya.
    </Typography>
    <div class="max-w-3xl">
