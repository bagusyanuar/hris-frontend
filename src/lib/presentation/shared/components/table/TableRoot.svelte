...
<div
  class={cn(
    'relative w-full overflow-x-auto rounded-xl border border-neutral-border bg-neutral-card',
    isLoading && hasNoData && 'min-h-[200px]'
  )}
>
  ...
  {#if isLoading}
    ...
    <div
      class="absolute inset-0 z-30 flex items-center justify-center bg-neutral-card/70 backdrop-blur-sm"
    >
      {#if loading}
        {@render loading()}
      {:else}
  ...
  {:else if hasNoData}
    ...
    {:else}
      <!-- Default Empty State -->
      <div
        class="flex flex-col items-center justify-center p-12 text-center border-t border-neutral-border"
      >
        <Icon icon="lucide:database" class="h-10 w-10 text-slate-400 mb-3" />
        <Typography variant="body-md" weight="medium">Tidak ada data</Typography>
