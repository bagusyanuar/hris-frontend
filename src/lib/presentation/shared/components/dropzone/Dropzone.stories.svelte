...
<Story name="Default">
	<div class="flex flex-col gap-4 max-w-xl">
		...
		{#if rejectedFilesList.length > 0}
			<div class="rounded-xl border border-rose-100 bg-rose-50/50 p-4 dark:border-rose-950/30 dark:bg-rose-950/10 flex flex-col gap-2">
				<div class="flex items-center justify-between">
					<span class="text-xs font-bold text-rose-600 dark:text-rose-400">File Gagal Upload:</span>
					<button onclick={clearRejections} class="text-xs text-rose-600 hover:underline font-medium">Clear</button>
				</div>
	...
	<div class="max-w-xl flex flex-col gap-4">
		...
		{#if customFiles.length > 0}
			<div class="p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50">
				<h4 class="text-xs font-bold text-slate-500 mb-2">Custom Render List ({customFiles.length} file):</h4>
				<div class="flex flex-wrap gap-2">
					{#each customFiles as file, index (file.name + '-' + index)}
						<div class="px-3 py-1.5 bg-brand-primary text-white rounded-full text-xs flex items-center gap-1.5">
							<span>{file.name}</span>
							<button 
