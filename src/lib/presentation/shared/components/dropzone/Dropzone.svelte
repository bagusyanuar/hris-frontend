<script lang="ts">
	...
	function getFileConfig(file: File): FileConfig {
		...
		if (['xls', 'xlsx', 'csv'].includes(ext)) {
			return { icon: 'lucide:file-spreadsheet', bgClass: 'bg-emerald-50 dark:bg-emerald-950/20', textClass: 'text-emerald-600 dark:text-emerald-400' };
		}
		if (['doc', 'docx'].includes(ext)) {
			return { icon: 'lucide:file-text', bgClass: 'bg-blue-50 dark:bg-blue-950/20', textClass: 'text-blue-600 dark:text-blue-400' };
		}
		if (ext === 'pdf') {
			return { icon: 'lucide:file-text', bgClass: 'bg-rose-50 dark:bg-rose-950/20', textClass: 'text-rose-600 dark:text-rose-400' };
		}
		if (['zip', 'rar', '7z', 'tar', 'gz'].includes(ext)) {
			return { icon: 'lucide:file-archive', bgClass: 'bg-amber-50 dark:bg-amber-950/20', textClass: 'text-amber-600 dark:text-amber-400' };
		}
		if (['mp3', 'wav', 'ogg'].includes(ext)) {
			return { icon: 'lucide:file-audio', bgClass: 'bg-indigo-50 dark:bg-indigo-950/20', textClass: 'text-indigo-600 dark:text-indigo-400' };
		}
		if (['mp4', 'mov', 'avi', 'mkv'].includes(ext)) {
			return { icon: 'lucide:file-video', bgClass: 'bg-purple-50 dark:bg-purple-950/20', textClass: 'text-purple-600 dark:text-purple-400' };
		}
		if (isImageFile(file)) {
			return { icon: 'lucide:file-image', bgClass: 'bg-emerald-50 dark:bg-emerald-950/20', textClass: 'text-emerald-600 dark:text-emerald-400' };
		}
		return { icon: 'lucide:file-x', bgClass: 'bg-slate-100 dark:bg-slate-800', textClass: 'text-slate-400 dark:text-slate-500' };
	}
	...
	function validateFiles(fileList: FileList): { accepted: File[]; rejected: FileRejection[] } {
		...
		filesArray.forEach((file) => {
			...
			if (currentFilesCount + accepted.length >= limit) {
				errors.push({ code: 'too-many-files', message: 'Jumlah file melebihi batas yang ditentukan' });
			}
			...
			if (maxSize && file.size > maxSize) {
				...
				errors.push({ code: 'file-too-large', message: `Ukuran file melebihi batas maksimal ${maxMb}MB` });
			}
...
<div class="flex w-full flex-col gap-4">
	<div
		...
		class={cn(
			...
			disabled ? 'opacity-50 cursor-not-allowed bg-slate-50 dark:bg-slate-900/20 border-slate-200' : 'cursor-pointer',
			className
		)}
			...
			<div class="flex flex-col items-center justify-center gap-3">
				...
				<div class="flex flex-col gap-1">
					<p class="text-sm font-medium text-slate-700 dark:text-slate-200">
						Tarik dan lepas file di sini, atau <span class="text-brand-primary hover:text-brand-hover hover:underline transition-colors font-semibold">pilih file</span>
					</p>
				...
				>
					...
					{#if isImageFile(file) && previewUrls[file.name + '-' + file.size]}
						...
						>
							...
							/>
							<div class="absolute inset-0 bg-slate-950/20 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex items-center justify-center text-white">
	...
	<Dialog bind:open={isModalOpen} title={activePreviewFile?.name} size="md" onclose={handleClosePreview}>
		{#if activePreviewFile}
			{#if isImageFile(activePreviewFile) && previewUrls[activePreviewFile.name + '-' + activePreviewFile.size]}
				<div class="flex items-center justify-center bg-slate-50 dark:bg-slate-900 rounded-xl p-2 max-h-[60vh] overflow-hidden">
					<img
			...
			{:else}
				...
				<div class="flex flex-col items-center justify-center p-6 text-center gap-4">
					<div class={cn("h-16 w-16 rounded-full flex items-center justify-center scale-110", config.bgClass, config.textClass)}>
						<Icon icon={config.icon} class="h-8 w-8" />
					</div>
					<div class="flex flex-col gap-1">
						<h3 class="text-sm font-semibold text-slate-800 dark:text-slate-200">Pratinjau Tidak Tersedia</h3>
						<p class="text-xs text-slate-500 dark:text-slate-400 max-w-xs">
							Format file <strong>{activePreviewFile.name.split('.').pop()?.toUpperCase()}</strong> tidak mendukung pratinjau langsung di browser.
						</p>
					</div>
