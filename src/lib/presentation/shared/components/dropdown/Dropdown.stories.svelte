...
<Story name="In Scrollable Table (Overflow Safe)">
	<div class="space-y-4">
		<p class="text-sm text-slate-500 max-w-lg">
			This table has <code>overflow-x-auto</code>. A standard absolute dropdown would be cut off by the table container, but because our dropdown is portaled to the document body, it floats correctly over the boundary.
		</p>
			...
			<table class="w-full text-left border-collapse min-w-[600px]">
				<thead>
					<tr class="border-b border-neutral-border bg-slate-50 dark:bg-slate-900/30">
						<th class="p-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Employee</th>
						...
						<th class="p-3 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Actions</th>
					</tr>
				</thead>
				<tbody>
					<tr class="border-b border-neutral-border last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/10">
						...
						<td class="p-3 text-sm">
							<span class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Active</span>
						</td>
