...
{#snippet container(isMobileView: boolean)}
	{@const ctx = {
		get isCollapsed() { return isMobileView ? false : isCollapsed; },
...
>
	...
	<div class="absolute top-4 right-4 md:hidden z-10">
		<button onclick={closeMobile} class="text-slate-500 hover:text-slate-800 p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer">
