...
<div class="w-full flex flex-col relative">
	...
	>
		...
		{#if sidebar.isCollapsed}
			<div class="absolute top-0 left-full pl-2 min-w-[188px] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto translate-x-2 group-hover:translate-x-0 transition-all duration-200 z-[999]">
				<div class="bg-white dark:bg-slate-950 border border-slate-100 dark:border-emerald-500/40 rounded-xl shadow-xl p-2 flex flex-col gap-1 w-full relative">
					<!-- Segitiga penunjuk -->
					<div class="absolute right-full top-5 -translate-y-1/2 border-y-4 border-y-transparent border-r-4 border-r-white dark:border-r-slate-950"></div>
					
					<div class="px-2 py-1 text-[9px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 pb-1.5 mb-1">
						{label}
					</div>
