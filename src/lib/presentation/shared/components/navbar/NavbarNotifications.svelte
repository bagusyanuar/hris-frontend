		...
		>
			...
			<div class="flex flex-col max-h-[300px] overflow-y-auto no-scrollbar">
				{#each notifications as notif (notif.id)}
					...
					>
						<div class="w-8 h-8 rounded-full {notif.iconBg} {notif.iconColor} flex items-center justify-center shrink-0 mt-0.5">
							<Icon icon={notif.icon} class="w-4 h-4" />
						</div>
						<div class="flex flex-col min-w-0">
							<span class="text-xs text-slate-800 dark:text-slate-200 font-medium">{notif.title}</span>
							<span class="text-[10px] text-slate-500 leading-tight mt-0.5">{notif.description}</span>
							<span class="text-[9px] mt-1.5 font-medium {notif.unread ? 'text-emerald-600' : 'text-slate-400'}">{notif.time}</span>
						</div>
					</button>
				{:else}
