...
{#snippet renderNode(row: Row<JobPositionModel & { children?: JobPositionModel[] }>, isLast: boolean, isFirst: boolean)}
	<div class="relative pt-3 first:pt-0">
		...
		{#if row.depth > 0}
			<div
				...
				style="left: -1.5rem; top: {isFirst ? '-1rem' : '0'}; height: {isLast ? (isFirst ? '3rem' : '2rem') : (isFirst ? 'calc(100% + 1rem)' : '100%')};"
			></div>
			<div
			...
			>
				<div class="flex flex-col gap-3">
					...
					<div class="flex items-center justify-between mt-1">
						<div class="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded-md border border-slate-100 dark:border-slate-800">
							<Icon icon="lucide:users" class="w-3.5 h-3.5" />
							Quota: {row.original.headcountQuota}
						</div>
						<Badge variant={row.original.status === 'active' ? 'success' : 'default'} class="text-[10px] px-1.5 py-0 font-medium">
...
<div class="p-6 bg-slate-50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-x-auto min-h-[400px]">
	<div class="min-w-fit flex flex-col pt-2 pb-8 px-4">
		{#if rootRows.length === 0}
			<div class="flex flex-col items-center justify-center py-16 text-center text-slate-500 w-full h-full">
