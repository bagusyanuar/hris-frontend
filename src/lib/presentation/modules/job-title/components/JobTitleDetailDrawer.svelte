	...
	{#if jobTitle}
		<div class="flex flex-col gap-6">
			...
			<div class="flex items-start gap-4">
				<div class="h-16 w-16 rounded-2xl bg-brand-light text-brand-primary flex items-center justify-center shrink-0">
					<Icon icon="lucide:award" class="w-8 h-8" />
				</div>
				<div class="flex flex-col gap-1.5 pt-1">
					<Typography variant="h4" weight="semibold">{jobTitle.name}</Typography>
					<div class="flex items-center gap-2">
						<span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600 font-mono dark:bg-slate-800 dark:text-slate-400 border border-slate-200 dark:border-slate-700 shadow-xs">
							{jobTitle.code}
						</span>
						<Badge variant={jobTitle.status === 'active' ? 'success' : 'default'}>
							<Icon
								icon={jobTitle.status === 'active' ? 'lucide:check-circle-2' : 'lucide:minus-circle'}
								class="w-3.5 h-3.5"
							/>
			...
			<div class="flex flex-col gap-4">
				...
				<div class="grid gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
					...
					<div class="flex flex-col gap-1">
						...
						<span class="text-sm font-medium text-slate-900 dark:text-slate-100">
							{jobTitle.createdAt ? new Date(jobTitle.createdAt).toLocaleDateString('id-ID', {
								day: 'numeric',
								month: 'long',
								year: 'numeric'
							}) : '-'}
