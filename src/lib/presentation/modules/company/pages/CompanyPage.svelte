...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-3 h-full items-start">
  ...
  <div class="lg:col-span-4 xl:col-span-3 flex flex-col gap-3 h-full">
    ...
    <Card class="p-4 pb-3.5">
      <div class="flex items-center justify-between gap-2 mb-3">
        <div class="flex items-start gap-2.5">
          <div
            class="h-8 w-8 rounded-lg bg-brand-light text-brand-primary flex items-center justify-center shrink-0 mt-0.5"
          >
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span
                class="text-[10px] font-medium text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-px rounded-full leading-none"
              >
    ...
    <div class="flex flex-col gap-1.5 max-h-[calc(100vh-240px)] overflow-y-auto pr-0.5">
      {#if dir.isLoading}
        <div
          class="flex items-center justify-center py-12 bg-neutral-card rounded-xl border border-neutral-border"
        >
      ...
      {:else}
        {#each dir.table.getRowModel().rows.map((r) => r.original) as company (company.id)}
          ...
          >
            ...
            {#if isActive}
              <div
                class="absolute left-0 top-2.5 bottom-2.5 w-0.75 bg-brand-primary rounded-r-full"
              ></div>
            {/if}

            <!-- Company icon -->
            <div
              class="h-9 w-9 rounded-lg flex items-center justify-center shrink-0 transition-all duration-200
              {isActive
            ...
            <div class="flex flex-col gap-0.5 min-w-0 flex-1">
              ...
              <div class="flex items-center gap-2 text-[11px] text-slate-400">
                <span
                  class="font-mono font-medium text-[10px] bg-slate-100 dark:bg-slate-800 px-1.5 py-px rounded"
                >
                  {company.code}
                </span>
              ...
              <Dropdown align="right">
                ...
                {#snippet content()}
                  ...
                  <DropdownItem
                    class="text-xs gap-2"
                    variant="danger"
                    onclick={() => dir.requestDeleteCompany(company)}
                  >
  ...
  <div class="lg:col-span-8 xl:col-span-9 flex flex-col gap-3 h-full">
    {#if !activeCompany}
      ...
      <Card class="flex flex-col items-center justify-center text-center py-24 min-h-100">
        <div class="relative mb-6">
          <div
            class="h-20 w-20 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center"
          >
            <Icon icon="lucide:building-2" class="w-9 h-9 text-slate-300" />
          </div>
          <div
            class="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-brand-light border-2 border-white dark:border-slate-950 flex items-center justify-center"
          >
    ...
    {:else}
      ...
      <div
        class="relative bg-neutral-card border border-neutral-border rounded-2xl overflow-hidden"
      >
        <!-- Decorative gradient background -->
        <div
          class="absolute top-0 left-0 right-0 h-28 bg-linear-to-br from-brand-primary/6 via-emerald-500/3 to-transparent dark:from-brand-primary/8 dark:via-transparent"
        ></div>
        ...
        <div class="relative z-10 p-6">
          ...
          <div class="flex items-start justify-between gap-4 mb-5">
            <div class="flex items-start gap-4">
              ...
              <div class="relative shrink-0">
                <div
                  class="h-14 w-14 rounded-2xl bg-linear-to-br from-brand-primary to-emerald-600 text-white flex items-center justify-center shadow-lg shadow-brand-primary/15"
                >
                ...
                {#if activeCompany.isActive}
                  <div
                    class="absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full bg-emerald-500 border-[2.5px] border-white dark:border-slate-900"
                  ></div>
                {/if}
              </div>
              ...
              <div class="flex flex-col gap-1 pt-0.5">
                <div class="flex items-center gap-2.5 flex-wrap">
                  <Typography
                    variant="h4"
                    weight="bold"
                    class="text-slate-900 dark:text-slate-50 tracking-tight leading-tight"
                  >
                    {activeCompany.legalName}
                  </Typography>
                  <span
                    class="text-[11px] font-mono font-semibold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md border border-slate-200/60 dark:border-slate-700/60"
                  >
                    {activeCompany.code}
                  </span>
                </div>
            ...
            <div class="flex items-center gap-2 shrink-0">
              <Button
                variant="outline"
                size="sm"
                onclick={() => dir.openEditCompany(activeCompany)}
              ...
              <Dropdown align="right">
                ...
                {#snippet content()}
                  <DropdownItem
                    class="text-xs gap-2"
                    variant="danger"
                    onclick={() => dir.requestDeleteCompany(activeCompany)}
                  >
                    <Icon icon="lucide:trash-2" class="w-3.5 h-3.5" />
                    Hapus Perusahaan
                  </DropdownItem>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div
              class="flex items-center gap-3 p-4 rounded-lg bg-slate-100/70 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60"
            >
              <div
                class="h-9 w-9 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-center shrink-0 shadow-sm"
              >
      ...
      <div class="bg-neutral-card border border-neutral-border rounded-2xl overflow-hidden">
        <!-- Section header -->
        <div
          class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800/60"
        >
          <div class="flex items-center gap-2.5">
            <div
              class="h-7 w-7 rounded-lg bg-brand-light dark:bg-brand-primary/10 text-brand-primary flex items-center justify-center"
            >
            ...
            <div class="flex items-center gap-2">
              ...
              <span
                class="text-[11px] font-medium text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full"
              >
          ...
          <Button
            ...
            class="h-8 gap-1.5 text-xs hover:bg-brand-light hover:border-brand-primary/30 hover:text-brand-primary"
          >
            <Icon icon="lucide:plus" class="w-3.5 h-3.5" />
            Tambah
          </Button>
        ...
        <div class="p-5">
          {#if (activeCompany.branches?.length ?? 0) === 0}
            ...
            >
              <div
                class="h-12 w-12 rounded-xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-center text-slate-300 group-hover:text-brand-primary group-hover:border-brand-primary/30 group-hover:scale-105 transition-all duration-300 mb-3"
              >
          ...
          {:else}
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              ...
              <button
                onclick={() => dir.openCreateBranch(activeCompany)}
                class="flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-brand-primary/40 bg-transparent hover:bg-brand-light/20 cursor-pointer transition-all duration-200 group min-h-30"
              >
                <div
                  class="h-9 w-9 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-brand-primary group-hover:bg-brand-light transition-all shrink-0"
                >
                ...
                <div class="text-left">
                  ...
                  <Typography variant="caption" color="secondary">Lokasi baru</Typography>
                </div>
              </button>
            </div>
          {/if}
