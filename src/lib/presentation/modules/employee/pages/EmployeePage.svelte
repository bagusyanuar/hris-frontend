...
<div class="space-y-6">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
    <button onclick={() => (activeTab = 'All')} class="text-left w-full">
      <Card
        hoverable={true}
        class="p-4 border border-neutral-border bg-neutral-card transition-all {activeTab === 'All'
    ...
    <button onclick={() => (activeTab = 'Active')} class="text-left w-full">
      <Card
        hoverable={true}
        class="p-4 border border-neutral-border bg-neutral-card transition-all {activeTab ===
        'Active'
          ? 'ring-2 ring-emerald-500/50'
          : ''}"
      >
        <Typography variant="caption" color="success">Active</Typography>
        <Typography variant="h5" weight="bold" class="mt-1 text-emerald-600 dark:text-emerald-500"
          >{activeCount}</Typography
        >
      </Card>
    </button>
    <button onclick={() => (activeTab = 'On Leave')} class="text-left w-full">
      ...
      >
        ...
        <Typography variant="h5" weight="bold" class="mt-1 text-amber-600 dark:text-amber-500"
          >{leaveCount}</Typography
        >
      </Card>
    </button>
    <button onclick={() => (activeTab = 'Inactive')} class="text-left w-full">
      ...
      >
        ...
        <Typography variant="h5" weight="bold" class="mt-1 text-slate-500"
          >{inactiveCount}</Typography
        >
      </Card>
    </button>
  </div>
  ...
  <Card class="bg-neutral-card border border-neutral-border p-6">
    ...
    <div
      class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-neutral-border mb-5"
    >
      <!-- Tab Switcher -->
      <div class="flex border-b border-slate-200 dark:border-slate-800">
        {#each ['All', 'Active', 'On Leave', 'Inactive'] as tab (tab)}
          <button
            ...
            class="px-4 py-2 text-xs font-medium border-b-2 transition-colors cursor-pointer {activeTab ===
            tab
              ? 'border-brand-primary text-brand-text dark:text-brand-primary'
            ...
            {#each filteredEmployees as emp (emp.id)}
              <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                <td class="py-3.5 px-4 flex items-center gap-3">
                  <div
                    class="w-9 h-9 rounded-full font-semibold flex items-center justify-center {emp.color}"
                  >
                    {emp.initials}
                  </div>
                ...
                <td class="py-3.5 px-4">
                  {#if emp.status === 'Active'}
                    <span
                      class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400 border border-emerald-200/50 dark:border-emerald-800/30"
                    >
                  ...
                  {:else if emp.status === 'On Leave'}
                    <span
                      class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 border border-amber-200/50 dark:border-amber-800/30"
                    >
                  ...
                  {:else}
                    <span
                      class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700"
                    >
    ...
    <!-- Footer Pagination -->
    <div
      class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-neutral-border pt-5"
    >
      <Typography variant="caption" color="secondary">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-semibold bg-brand-light text-brand-text dark:bg-emerald-500/10 dark:text-emerald-400"
        >
          1
        </button>
        <Button variant="outline" size="sm" class="h-8 py-0 px-2" disabled={true}>
