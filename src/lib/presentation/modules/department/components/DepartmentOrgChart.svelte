...
<div
  class="overflow-x-auto p-4 sm:p-8 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-200 dark:border-slate-800 min-h-[500px]"
>
  {#if departments.length === 0}
    <div class="flex flex-col items-center justify-center h-full text-slate-500 py-12">
      ...
      <Typography variant="body-md"
        >Tidak ada departemen untuk ditampilkan di Bagan Organisasi.</Typography
      >
    </div>
  {:else}
  ...
  <div class="flex flex-col relative {depth > 0 ? 'ml-12' : ''}">
    ...
    >
      <div class="flex items-center justify-between mb-2">
        <span
          class="text-xs font-mono font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md"
        >
          {dept.code}
        </span>
        <div
          class={`w-2 h-2 rounded-full ${dept.status === 'active' ? 'bg-emerald-500' : 'bg-slate-300'}`}
        ></div>
      </div>
      ...
      <Typography
        variant="h6"
        weight="medium"
        class="mb-1 text-slate-900 dark:text-slate-100 line-clamp-1"
      >
        {dept.name}
      </Typography>

      <div
        class="mt-3 flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-3"
      >
        <div class="flex items-center gap-2">
          <Avatar name={dept.managerName} size="xs" variant="primary" />
          <span
            class="text-xs text-slate-600 dark:text-slate-400 font-medium truncate max-w-[80px]"
          >
