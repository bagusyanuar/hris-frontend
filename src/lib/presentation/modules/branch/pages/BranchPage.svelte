<script lang="ts">
  ...
  import BranchDetailDrawer from '../components/BranchDetailDrawer.svelte';
  import { useBranchDirectory } from '../runes/branch-directory.svelte';

  const dir = useBranchDirectory();
</script>
...
<div class="flex flex-col gap-3">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full">
      <div class="flex items-center gap-4">
        ...
        <div class="flex flex-col gap-1">
          <Typography variant="h4" weight="bold">Cabang</Typography>
          <Typography variant="body-sm" color="secondary">
            Kelola data master cabang dan struktur anak perusahaan.
          </Typography>
        </div>
      </div>
      <div
      ...
      >
        <div class="flex items-center gap-2 px-3">
          <Icon icon="lucide:building-2" class="w-4 h-4 text-slate-400" />
          <span class="text-sm font-medium text-slate-700 dark:text-slate-300">
            {dir.stats.total} <span class="text-slate-500 font-normal">Cabang</span>
          </span>
        </div>
  ...
  <Card>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
      <Typography variant="h5" weight="semibold">Daftar Cabang</Typography>
              ...
              <TableUI.Cell {cell}>
                {#if cell.column.id === 'name'}
                  <div class="flex items-center gap-2">
                    <Icon icon="lucide:building-2" class="w-4 h-4 text-slate-400 shrink-0" />
                ...
                {:else if cell.column.id === 'address'}
                  <span
                    class="text-slate-500 dark:text-slate-400 text-sm truncate max-w-[250px] block"
