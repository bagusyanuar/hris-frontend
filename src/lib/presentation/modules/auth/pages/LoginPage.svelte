...
<div class="min-h-screen bg-neutral-bg flex lg:grid lg:grid-cols-12 font-sans transition-colors duration-300">
	<!-- Left Side: Brand Visuals (Only on large screens) -->
	<div class="relative hidden lg:flex lg:col-span-7 bg-slate-950 flex-col justify-between p-12 overflow-hidden border-r border-neutral-border">
		...
		<div class="relative z-10 flex items-center gap-3">
			<div class="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-primary text-white shadow-lg shadow-brand-primary/20">
		...
		<div class="relative z-10 my-auto max-w-lg space-y-6">
			...
			<Typography variant="body-lg" color="inherit" class="text-slate-300">
				An elegant, all-in-one corporate portal to streamline payroll, attendance, leaves, and talent management with enterprise-grade security.
			</Typography>

			<!-- Premium Badge card -->
			<div class="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6 shadow-2xl transition-all duration-300 hover:border-white/20">
				<div class="flex items-start gap-4">
					<div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-brand-primary/20 text-brand-primary">
					...
					<div class="space-y-1">
						...
						<Typography variant="body-sm" color="inherit" class="text-slate-400">
							Experience automated talent mapping, smart scheduling, and real-time dashboard generation.
						</Typography>
					</div>
				</div>
			</div>
	...
	<div class="flex-1 lg:col-span-5 flex flex-col justify-center items-center px-4 sm:px-6 lg:px-12 xl:px-16 py-12 relative overflow-hidden bg-neutral-bg">
		<!-- Decorative Background Ambient Glow for Mobile/Tablet -->
		<div class="lg:hidden absolute top-0 right-0 w-80 h-80 bg-brand-primary rounded-full filter blur-3xl opacity-10"></div>
		<div class="lg:hidden absolute bottom-0 left-0 w-80 h-80 bg-emerald-500 rounded-full filter blur-3xl opacity-5"></div>

		<div class="w-full max-w-md space-y-8 relative z-10">
			<!-- Mobile Logo Header -->
			<div class="flex flex-col items-center text-center lg:items-start lg:text-left">
				<div class="flex lg:hidden h-12 w-12 items-center justify-center rounded-2xl bg-brand-primary text-white shadow-lg shadow-brand-primary/20 mb-4">
					<Icon icon="lucide:layers" class="h-6 w-6" />
				</div>
				<Typography variant="h3" weight="bold" class="tracking-tight text-slate-900 dark:text-slate-100">
					Welcome back
				</Typography>
			...
			<!-- Login Card -->
			<div class="rounded-2xl border border-neutral-border bg-neutral-card p-6 sm:p-8 shadow-sm transition-all duration-300 hover:shadow-md">
				{#if store.isAuthenticated}
					<!-- Authenticated State -->
					<div class="text-center space-y-6 py-4">
						<div class="w-16 h-16 bg-brand-light text-brand-text rounded-full flex items-center justify-center mx-auto text-3xl shadow-sm border border-brand-border">
						...
						<div class="space-y-2">
							...
							<Typography variant="body-md" color="muted">
								Anda berhasil masuk sebagai <span class="font-semibold text-slate-900 dark:text-slate-100">{store.user?.name}</span> ({store.user?.role})
							</Typography>
						</div>
				...
				{:else}
					...
					<form class="space-y-5" onsubmit={handleSubmit}>
						...
						<!-- Alert Error -->
						{#if store.error}
							<div class="bg-rose-50 dark:bg-rose-950/30 border-l-4 border-rose-500 p-4 rounded-xl transition-all duration-200">
								<div class="flex">
						...
						<div class="flex items-center justify-between pt-1">
							...
							<a href="#forgot" class="hover:underline text-brand-primary transition-all duration-150">
			...
			<div class="rounded-xl bg-slate-100 dark:bg-slate-900 border border-neutral-border p-4 text-center">
				<Typography variant="caption" color="muted">
					Hint: Gunakan <code class="bg-white dark:bg-slate-950 border border-neutral-border px-1.5 py-0.5 rounded text-slate-800 dark:text-slate-200 font-mono">admin@hris.com</code> dan <code class="bg-white dark:bg-slate-950 border border-neutral-border px-1.5 py-0.5 rounded text-slate-800 dark:text-slate-200 font-mono">password123</code>
				</Typography>
			</div>
		</div>
	</div>
