						...
						>
							...
							{#if !isCollapsed}
								<Icon
									...
									class="w-3.5 h-3.5 text-slate-400 transition-transform duration-200 {isPayrollExpanded ? 'rotate-180' : ''}"
								/>
							{/if}
						</button>
	...
	<div class="flex-1 flex flex-col min-w-0 gap-3 overflow-hidden">
		...
		>
			...
			<div class="flex items-center gap-3">
				...
				<!-- Search Bar (Desktop) -->
				<div class="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-900/40 border border-neutral-border rounded-xl w-64 focus-within:w-80 focus-within:border-emerald-500/50 transition-all duration-300 ml-4 relative">
							...
							{:else}
								...
								<div>
									...
									<div class="flex flex-col gap-0.5 mt-1">
										...
										].filter(r => r.name.toLowerCase().includes(searchQuery.toLowerCase()) || r.type.toLowerCase().includes(searchQuery.toLowerCase())) as res (res.name)}
											<button
												onclick={() => {
		...
		<!-- PAGE BODY -->
		<main class="flex-1 overflow-y-auto bg-neutral-card border border-neutral-border rounded-xl p-6 md:p-8 space-y-6 scrollable-content">
			<!-- Welcome Header -->
