...
<main
  class="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 font-sans selection:bg-indigo-500 selection:text-white"
>
  <div class="max-w-4xl mx-auto space-y-8">
    <!-- Header -->
    <header
      class="border-b border-slate-800 pb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4"
    >
      <div>
        <h1
          class="text-4xl font-extrabold tracking-tight bg-gradient-to-r from-orange-500 via-pink-500 to-indigo-500 bg-clip-text text-transparent"
        >
          Belajar Svelte 5 🚀
        </h1>
        <p class="text-slate-400 mt-2">
          Selamat datang di playground interaktif Svelte 5 menggunakan Runes.
        </p>
      </div>
      <div
        class="flex items-center gap-3 bg-slate-900 border border-slate-800 px-4 py-2 rounded-full w-fit"
      >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <section class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs bg-slate-800 text-slate-400 px-2.5 py-1 rounded-md font-mono"
            >$state & bind:value</span
          >
        </div>

        <!-- Input Binding -->
        <div class="space-y-2">
          <label for="name-input" class="block text-sm font-medium text-slate-400"
            >Siapa namamu?</label
          >
          <input
            id="name-input"
        ...
        <div class="bg-slate-950/50 rounded-xl p-4 border border-slate-800/50">
          ...
          <p class="text-sm text-slate-500 mt-1 font-mono">
            Panjang nama: {nameLength} karakter (dihitung via
            <span class="text-pink-500">$derived</span>)
          </p>
        </div>
        ...
        <div class="space-y-4 pt-4 border-t border-slate-800">
          ...
          <div class="flex items-center gap-4">
            ...
            <div
              class="flex-1 text-center bg-slate-950 border border-slate-850 rounded-xl py-2 font-mono text-2xl font-bold"
            >
              {counter}
            </div>
            <button
          ...
          <div class="bg-indigo-950/20 border border-indigo-900/30 rounded-xl p-3 text-center">
            <span class="text-xs text-indigo-400 font-mono"
              >Nilai Kuadrat ({counter}²): {counterSquared}</span
            >
          </div>
        </div>
      </section>
      ...
      <section
        class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6 flex flex-col justify-between"
      >
        <div class="space-y-6">
          <div class="flex items-center justify-between">
          ...
          <div class="space-y-2">
            <div class="flex justify-between text-xs text-slate-400">
              ...
              <span class="font-mono"
                >{completionPercentage}% ({completedCount}/{todos.length})</span
              >
            </div>
            <div
            ...
            >
              <div
                class="bg-gradient-to-r from-orange-500 to-pink-500 h-full transition-all duration-300"
          ...
          <ul class="space-y-2 max-h-[220px] overflow-y-auto pr-1">
            {#each todos as todo (todo.id)}
              <li
                class="flex items-center justify-between bg-slate-950 p-3 rounded-xl border border-slate-850 hover:border-slate-800 transition"
              >
    ...
    <footer class="text-center pt-8 text-xs text-slate-600 border-t border-slate-900">
      Dibuat untuk belajar Svelte 5 Runes • Akses halaman ini di browser pada alamat <code
        class="bg-slate-900 px-1.5 py-0.5 rounded text-slate-400">/belajar</code
      >
    </footer>
  </div>
</main>
