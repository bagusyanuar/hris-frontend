...
* **Separation of Concerns:** TanStack Table mengurus *state* (sorting, pagination, filtering, column visibility, row selection), sedangkan Svelte component kita fokus pada render elemen HTML & style (Tailwind v4).
...
```svelte
<Table.Cell class={cell.getValue() === 'Active' ? 'text-emerald-600 font-semibold' : 'text-slate-500'}>
  {cell.getValue()}
</Table.Cell>
```
